...
\begin{proof}
  To prove the unconditional summability and the norm estimates, it suffices to consider functions $\mb{f}$ with finite Haar expansions, as the subspace of all such functions is dense in $L^p(\R;X)$.\todo{link to proof of this}
...
\todo{put the notation $S^{\omega,t}_{k}$ above where necessary}
...
\begin{lem}
  ...
  \end{equation*}
  pointwise for a.e. $x \in \R$, and in $L^p(\R;X)$.\todo{put note on sense of series $\sum_{-\infty}^{\infty}$}
\end{lem}
...
\begin{lem}
  \todo{averaging over $\omega$ gives a convolution operator}
\end{lem}

\todo{discussion of average dyadic shifts}

\begin{prop}
  \todo{consequential boundedness}
\end{prop}
