...
\begin{cor}\label{cor:strong-meas-meas-mult}
  Suppose that $\map{f}{S}{X}$ is strongly measurable and the scalar-valued function $\map{\phi}{S}{\C}$ is measurable.
...
Given a measure space $(S,\mc{A},\mu)$ and a Banach space $X$, every function $g \in L^{p'}(S;X^*)$ induces a bounded linear functional $\Phi_g \in L^p(S;X)^*$ by integration of the $X-X^*$ duality pairing:
\begin{equation*}
  \Phi_g(f) := \int_S \langle f(s), g(s) \rangle \, \dd\mu(s) \qquad \forall f \in L^p(S;X).
\end{equation*}
H\"older's inequality implies that $\|\Phi_g\|_{L^p(S;X)^*} \leq \|g\|_{L^{p'}(S;X^*)}$.
In the scalar case $X = \C$, $\Phi$ is an isometric isomorphism $L^{p'}(S) \cong L^p(S)^*$: that is, every functional $\phi \in L^p(S)^*$ is of the form $\phi = \Phi_g$ for some $g \in L^{p'}(S)$, and furthermore $\|\phi\|_{L^p(S)^*} = \|g\|_{L^{p'}(S)}$.
We will see later on that this is true for a Banach space $X$ if and only if it has the \emph{Radon--Nikodym property}.
...
\begin{proof}
  To show that $\Phi$ is an isometry, it suffices to show that $\|\Phi_g\|_{L^p(S;X)^*} \geq 1$ whenever $g \in L^{p'}(S;X^*)$ with $\|g\|_{L^{p'}(S;X^*)} = 1$ (the reverse estimate was already mentioned; it follows from H\"older's inequality).
  ...
  Testing $\Phi_g$ against $f_{\varepsilon}$ yields
  \begin{equation*}
    \begin{aligned}
      \Phi_g(f_\varepsilon) = \sum_{n=1}^N \mu(S_n) \|\mb{x}_n^*\|_{X^*}^{p' - 1} \langle \mb{x}_n, \mb{x}_n^* \rangle
      &\geq (1-\varepsilon) \sum_{n=1}^N \mu(S_n) \|\mb{x}_n^*\|_{X^*}^{p'} \\
      &= (1-\varepsilon) \|g\|_{L^{p'}(S;X^*)} = 1-\varepsilon.
    \end{aligned}
  \end{equation*}
  This proves that $\|\Phi_g\|_{L^p(S;X)^*} \geq 1-\varepsilon$.
  Since $\varepsilon > 0$ was arbitrary, we find that $\|\Phi_g\|_{L^p(S;X)^*} \geq 1$ as intended.
  ...
  \begin{equation*}
    \begin{aligned}
      |\Phi_g(f)| &=  \Big| \fint_{B_{\varepsilon, \mb{x}^*}} \langle \mb{x}, g(s) \rangle \, \dd\mu(s) \Big| \\
    ...
    \end{aligned}
  \end{equation*}
  Since $\varepsilon > 0$ was arbitrary, we get $\|\Phi_g\|_{L^1(S;X)^*} \geq 1$, as we wanted.

  \textbf{Norming property:}
  Note that since $X$ embeds isometrically into its double dual $X^{**}$, we can identify $X$-valued functions with $X^{**}$-valued functions which take values in $X$.
  We exploit this and the previous results to show
  \begin{equation*}
    \begin{aligned}
      \|f\|_{L^p(S;X)} = \|f\|_{L^p(S;(X^*)^*)} &= \|\Phi_f\|_{L^{p'}(S; X^*)^*} \\
...
\begin{defn}
  For a measurable space $(S,\mc{A})$ and a set $V \subset \Meas(S;\C)$ of measurable scalar-valued functions on $S$, we define the \emph{algebraic tensor product}
  \begin{equation*}
...
\begin{proof}
  ...
  For density, note that $L^p(S) \otimes X$ contains $(\Sigma(S;\C) \cap L^p(S)) \otimes X$, and that
  \begin{equation*}
    (\Sigma(S;\C) \cap L^p(S)) \otimes X = \Sigma(S;X) \cap L^p(S;X),
  \end{equation*}
...
\begin{defn}\label{defn:tensor-exts}
  Let $(S,\mc{A},\mu)$ be a measure space, $p,q \in [1,\infty)$, and consider a bounded linear operator
  \begin{equation*}
    \map{T}{L^p(S,\mu)}{L^q(S,\mu)}
  \end{equation*}
  ...
  \begin{equation*}
    \map{T \otimes I}{L^p(S,\mu) \otimes X}{L^q(S,\mu) \otimes X}
  \end{equation*}
  satisfying $(T \otimes I)(f \otimes \mb{x}) = (Tf) \otimes \mb{x}$ for all $f \in L^p(S,\mu)$ and $\mb{x} \in X$.
  Often we will denote $T \otimes I$ by $\td{T}$ or (overloading notation) $T$.
\end{defn}

The tensor extension is a well-defined map between algebraic tensor products $L^p(S) \otimes X \to L^q(S) \otimes X$.
By Proposition \ref{prop:ATP-density}, these are dense subspaces of $L^p(S;X)$ and $L^q(S;X)$ respectively, so if there exists $C < \infty$ such that
\begin{equation}\label{eq:tensor-ext-estimate}
  \|(T \otimes I)f\|_{L^q(S;X)} \leq C \|f\|_{L^p(S;X)} \qquad \forall f \in L^p(S) \otimes X,
\end{equation}
then $T \otimes I$ may be extended to a bounded linear operator $L^p(S;X) \to L^q(S;X)$.
Writing out a general element $f \in L^p(S) \otimes X$ as a linear combination of elementary tensors, we see that the estimate \eqref{eq:tensor-ext-estimate} is equivalent to
\begin{equation}\label{eq:tensor-ext-estimate-full}
  \Big\|\sum_{n=1}^N (Tf_n) \otimes \mb{x}_n\Big\|_{L^q(S;X)} \leq C \Big\|\sum_{n=1}^N f_n \otimes \mb{x}_n\Big\|_{L^p(S;X)} \qquad \forall f_n \in L^p(S) \quad \forall \mb{x}_n \in X.
\end{equation}
...
\textbf{The goal of this course is to understand these interactions for various operators and Banach spaces.}

We have already seen one fundamental example.
\begin{example}
  Fix a measure space $(S,\mc{A},\mu)$ and let $\map{T}{L^1(S)}{\C}$ denote the Lebesgue integral.\footnote{This fits in the scope of Definition \ref{defn:tensor-exts} by considering $\C$ as a Lebesgue space $L^1(\mathrm{pt})$ over a single point, with counting measure. Then $X$ may be identified with the Bochner space $L^1(\mathrm{pt};X)$.}
  Let $X$ be a Banach space.
  Then for all $f \in \Simp(S;\C) \otimes X$ we have
  \begin{equation*}
  ...
  \end{equation*}
  so that the tensor extension of the Lebesgue integral is the Bochner integral, which we have already shown extends to a bounded linear map $L^1(S;X) \to X$.
\end{example}

In this example, the Banach space $X$ plays no real role; we will see in Proposition \ref{prop:positive-extensions} that this phenomenon occurs for all positive operators.
Before that we record a simple observation: tensor extensions can do no better than the original operator.

\begin{prop}
  Fix a measure space $(S,\mc{A},\mu)$ and $p,q \in [1,\infty)$.
  Let $T \in \Lin(L^p(S),L^q(S))$ be a bounded linear operator, and let $X$ be a Banach space.
  Then the tensor extension $\td{T}$ satisfies
  \begin{equation*}
    \|\td{T}\|_{L^p(S;X) \to L^q(S;X)} \geq \|T\|_{L^p(S) \to L^q(S)}.
  \end{equation*}
\end{prop}

\begin{proof}
  Fix a nonzero vector $\mb{x} \in X$.
  Then for all nonzero $f \in L^p(S)$ we have
  \begin{equation*}
    \begin{aligned}
      \|\td{T}(f \otimes \mb{x})\|_{L^q(S;X)} = \|Tf \otimes \mb{x}\|_{L^q(S;X)} &= \|Tf\|_{L^q(S)} \|\mb{x}\|_X \\
      &= \frac{\|Tf\|_{L^q(S)}}{\|f\|_{L^p(S)}} \|f \otimes \mb{x}\|_{L^p(S;X)}.
    \end{aligned}
  \end{equation*}
...
\begin{thm}\label{thm:positive-extensions}
  Fix a measure space $(S,\mc{A},\mu)$ and $p,q \in [1,\infty)$.
  Let $T \in \Lin(L^p(S),L^q(S))$ be a bounded linear operator which is \emph{positive}, meaning that for all (a.e.) nonnegative $f \in L^p(S)$, $Tf \in L^q(S)$ is also (a.e.) nonnegative.
  Then for every Banach space $X$ the tensor extension $\td{T}$ is bounded from $L^p(S;X) \to L^q(S;X)$, and in fact
  \begin{equation*}
    \|\td{T}\|_{L^p(S;X) \to L^q(S;X)} = \|T\|_{L^p(S) \to L^q(S)}.
  \end{equation*}
\end{thm}
...
\begin{proof}
  ...
  \end{equation}
  for all $f \in \Simp(S;X) \cap L^p(S;X)$ and almost all $s \in S$.
  This will imply
  \begin{equation*}
    \begin{aligned}
      \|\td{T}f\|_{L^q(S;X)} &= \Big( \int_{S} \|\td{T}f(s)\|_X^q \, \dd\mu(s) \Big)^{1/q} \\
      &\leq \Big( \int_{S} T(\|f(s)\|_X)^q \, \dd\mu(s) \Big)^{1/q}\\
      &\leq \|T\|_{L^p(S) \to L^q(S)} \Big( \int_{S} \|f(s)\|_X^p \, \dd\mu(s) \Big)^{1/p} \\
      &= \|T\|_{L^p(S) \to L^q(S)} \|f\|_{L^p(S;X)}
    \end{aligned}
  \end{equation*}
  and complete the proof by density of $\Simp(S;X) \cap L^p(S;X)$ in $L^p(S;X)$.
...
Since most interesting operators are not positive, a lot of work remains.
...
\begin{exercise}\label{ex:tensor-adjoint}
  Let $(S,\mc{A},\mu)$ be a measure space, let $T$ be a bounded linear operator on $L^2(S,\mu)$, and let $X$ be a Banach space.
  Let $T^*$ denote the adjoint of $T$.
  Show that $\td{T} = T \otimes I$ is bounded on $L^2(S,\mu;X)$ if and only if $\td{T^*} = T^* \otimes I$ is bounded on $L^2(S,\mu;X^*)$, and in this case their bounded linear extensions to $L^2(S,\mu;X)$ and $L^2(S,\mu;X^*)$ satisfy $(\td{T})^* = \td{T^*}$.
\end{exercise}
...
\begin{exercise}
  ...
  \item Show that the Fourier transform on the torus, initially defined as a bounded operator $\map{\mc{F}}{L^1(\T^d;H)}{\ell^\infty(\Z^d;H)}$, extends to an isometry from $L^2(\T^d;H)$ to $\ell^2(\Z^d;H)$.
  \end{itemize}
\end{exercise}
