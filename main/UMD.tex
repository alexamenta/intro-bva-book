...
\begin{prop}\label{prop:haar-multipliers}
...
\begin{proof}
  ...
  We are left with estimating the square function of $g_{\bullet}$ from below.
  % Observe that
  % \begin{equation*}
  %   g_{k}(\omega) = 2^{k}\1_{\{\omega_{1} = \cdots = \omega_{k} = 1\}} \quad \text{and} \quad dg_{k}(\omega) = \pi_{k}(\omega) dg_{k-1}(\omega)
  % \end{equation*}
  % for all $\omega \in \Omega$.
  Define sets
  \begin{equation*}
    \begin{aligned}
      \Omega_{0} &:= \{\omega \in \Omega: \omega_{1} = -1\}, \\
      \Omega_{k} &:= \{\omega_{1} = \cdots = \omega_{k} = 1, \omega_{k+1} = -1\} \qquad (0 < k < N), \\
      \Omega_{N} &:= \{\omega_{1} = \cdots = \omega_{N} = 1\}.
    \end{aligned}
  \end{equation*}
  These sets partition $\Omega$, so we have
  \begin{equation*}
    \begin{aligned}
    \Big\| \Big( \sum_{n=0}^{N} |dg_{n}|^2 \Big)^{1/2} \Big\|_{L^1(\Omega)}
    &= \sum_{m = 0}^{N} \int_{\Omega_{m}} \Big( \sum_{n=0}^{N} |dg_{n}(\omega)|^2 \Big)^{1/2} \, \dd\P(\omega) \\
  ...
  \end{equation*}
  for $n > 1$, and $dg_{0}(\omega) \equiv 1$, so carefully working with the definition of the sets $\Omega_{m}$ yields
  \begin{equation*}
    \begin{aligned}
