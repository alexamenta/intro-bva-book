...
\begin{equation*}
  \map{\mb{f}}{\R^{d}}{X},
\end{equation*}
where $X$ is a Banach space, and we will study properties of operators acting on $X$-valued functions.
...
\section{A case study: the Fourier transform and Plancherel's theorem}
...
\end{equation*}
so every $X$-valued function $\map{\mb{f}}{\R}{X}$ may be written as
\begin{equation*}
  \mb{f}(x) = \sum_{n=1}^{N} f_{n}(x) \mb{e}_{n}
\end{equation*}
for some scalar-valued functions $\map{f_{n}}{\R}{\C}$.
This expansion suggests that properties of $\mb{f}$ can be deduced from properties of the scalar-valued functions $f_{n}$.

The Fourier transform is defined on integrable scalar-valued functions $f \in L^1(\R)$ by
\begin{equation*}
...
Different operators (and different values of $p$) can be used to detect different geometric properties.

\section{A case study: the Hilbert transform}

One important operator in harmonic analysis is the Hilbert transform, defined on $\map{f}{\R}{\C}$ by
...
\end{equation}
The Hilbert transform is a prototypical \emph{singular integral operator}: the integral kernel $1/y$ is not integrable, so $H$ cannot be defined as a classical integral operator, but cancellation between the positive and negative values of $1/y$ allow $Hf$ to be well-defined when $f$ is sufficiently smooth (Schwartz, for example).
...
\begin{equation*}
  \|Hf\|_{L^2(\R;X)} \leq K_{X} \|m\hat{f}\|_{L^2(\R;X)} \leq K_{X} \|m\|_{L^\infty(\R)} \|\hat{f}\|_{L^2(\R;X)}= K_{X}^{2} \|f\|_{L^{2}(\R;X)},
\end{equation*}
where $1 \leq K_{X} < \infty$ denotes the norm of the Fourier transform on $L^2(\R;X)$.
Vector-valued Calder\'on--Zygmund methods can then be used to extrapolate this result to $L^p(\R;X)$-boundedness for all $p \in (1,\infty)$.

If $X$ is not isomorphic to a Hilbert space, then we know by Kwapie\`n's theorem that the Fourier transform is \emph{not} bounded on $L^2(\R;X)$, so the previous argument is unavailable.
However, the scalar-valued Hilbert transform is bounded on $L^p(\R)$ for all $p \in (1,\infty)$, not just $p = 2$, which suggests that boundedness of the $X$-valued Hilbert transform is a less stringent condition than the $X$-valued Plancherel theorem.
And indeed, this is the case.
We will prove the following theorem in Chapter \ref{sec:HT}.

\begin{thm}[Burkholder--Bourgain, 1981]
...
UMD stands for Unconditionality of Martingale Differences.
...
Even spaces of operators can be UMD, for example the Schatten classes $\mc{S}^{p} \subset B(H)$ (studied in Chapter \ref{sec:schatten}) or more general \emph{noncommutative} $L^p$-spaces (unfortunately not covered in these notes), and consequences of the UMD property for these spaces can be used to prove deep results in operator theory.

\section{Plan of attack}

In this course you will be introduced to some fundamental topics in Banach-valued analysis.\footnote{One very important topic which I have completely skipped is the relationship between weighted norm estimates, the Hardy--Littlewood maximal operator, and the UMD property for Banach function spaces and lattices. These topics are completely fundamental and should be included in an introductory course, but then I wouldn't have time to include the material on Schatten classes, and after all, the future is noncommutative...}

We start in Chapter \ref{sec:Bochner-spaces} with Bochner spaces and strong measurability.
Measurability is always a bit of a pain to discuss but it would be impossible to do anything without it, and in the infinite-dimensional setting a bit of preliminary work is needed so that we can thoughtlessly integrate.

We move on to probabilistic concepts in Chapter \ref{sec:martingales}, the most important being that of a vector-valued martingale.
As described above, this is needed in order to define the UMD property, but it is fundamental in its own right, and it is very useful in analysing the \emph{Radon-Nikodym property}.
This is done in Chapter \ref{sec:RNP}.
