...
\begin{proof}
  ...
  Now consider a fixed $A' \in \mc{A}_{+}(A)$ and let $\{B_{\lambda}\}_{\lambda \in \Lambda}$ be a maximal collection of disjoint measurable elements of $\mc{A}_+(A')$ such that $\|\mb{x}_{A'} - \mb{x}_{B_\lambda}\|_{X} > \varepsilon$, where $\Lambda$ is some indexing set.
  ...
  Taking the supremum over $A \in \mc{A}_{+}$ proves \eqref{eq:g-computation} and completes the proof.
\end{proof}

Combining this with everything else we know, we have proven the following theorem.
\begin{thm}\index{Radon--Nikodym property!equivalent characterisations}
