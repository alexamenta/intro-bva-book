...
\begin{proof}
  ...
  Then for each $k \in \N$ we have\todo{make sure this property is listed somewhere}
  \begin{equation*}
   ...
   \end{equation*}
   Since the functions $\E^{\mc{A}_k}(\1_{A}) - \E^{\mc{A}_\ell}( \1_{A})$ are uniformly in $L^\infty$ and the set $(f_{n})_{n \in \N}$ is uniformly integrable,\todo{up to here. have to do the proof using UI directly, doesn't seem to follow abstractly from weak compactness.}
\end{proof}
